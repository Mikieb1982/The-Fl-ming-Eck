...
import { englishHeuristic, extractTextFromArticleBody } from '../utils/helpers';
...
function validateArticles(articles: Article[], now: Date = new Date("2025-08-27T00:00:00Z"), maxAgeDays: number = 45): ValidationResult {
  const checks: ValidationCheck[] = articles.map((a) => {
    ...
    const articleText = extractTextFromArticleBody(a.body);
    const english = englishHeuristic(`${a.title} ${articleText}`);
